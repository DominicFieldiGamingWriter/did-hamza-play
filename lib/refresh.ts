...
export async function refreshPlayerPage() {
  ...
  if (!playerId) {
    ...
    const exact = matches.find(
      m => m.player.name.toLowerCase() === name.toLowerCase()
    );
  ...
  const playerMatches = await getPlayer(playerId, season);
  const player = playerMatches.find(m => m.player.id === playerId);

  const team = player?.statistics?.find(s => s.team?.id)?.team;

  if (!team?.id) {
    throw new Error("Could not determine the player's current team.");
  }

  const { last, next } = await getTeamFixtures(team.id);

  if (!last) {
    throw new Error("Could not find a completed/latest fixture.");
  }
